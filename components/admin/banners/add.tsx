...
import { adminCategories, categoryRequest } from "../../../api/admin/category"
...
const CatPage = () => {
	...
	const isLoadingPage = useSelector(
		(state: any) => state.generalReducer.isLoadingPage
	)
	const isDataSuccess = useSelector(
		(state: any) => state.generalReducer.isDataSuccess
	)
	const banner = useSelector((state: any) => state.bannerReducer.banner)
	...
	const childCategories = useSelector(
		(state: any) => state.categoryReducer.categories2
	)
	const childCategories2 = useSelector(
		(state: any) => state.categoryReducer.categories3
	)

	const router = useRouter()
	const pathname = useRouter().pathname
	const path_type =
		useRouter().pathname == "/hamyar-web/admin/banners/add" ? "add" : "edit"
	...
	useEffect(() => {
		if (router.query.id) bannerRequest(dispatch, router.query.id)
	}, [router.isReady])

	useEffect(() => {
		if (banner.title) {
			setDescription(banner.body)
	...
	}, [isDataSuccess, router.isReady, category, isLoadingPage])
	...
	const handleCatParent = (e: any, level: number) => {
		...
		if (e.target.value > 0)
			adminCategories(dispatch, {
				...
				child_id: level == 1 ? 0 : e.target.value,
			})
	}
	const formik: any = useFormik({
		...
		onSubmit: (values, { resetForm }) => {
			let data: any = {
				...
				image: image,
				delete_image:
					src == "" && category.image && category.image != "" ? true : false,
				parent_id:
					child_id2 > 0 ? child_id2 : child_id > 0 ? child_id : parent_id,
			}
			if (!isLoading) {
				if (router.query.id) adminUpdateCategory(dispatch, data)
				else adminAddCategory(dispatch, data)
			}
	...
	return (
		<CatPageEl>
			...
			{isLoadingPage ? (
				...
				<ContentEl>
					<Title>{path_type == "add" ? "افزودن دسته" : "ویرایش دسته"}</Title>
					...
					<form onSubmit={formik.handleSubmit}>
						...
						<InputEl>
							...
							{category && category.image != "" && src == category.image ? (
								<CustomImageUploaded
									src={src}
									handleSrc={(e: any) => setSrc(e)}
								/>
							) : (
								<CustomFileUploadEl
									src={src}
									handleSrc={(e: any) => setSrc(e)}
									handleEvent={(e: any) => setImage(e)}
								/>
							)}
						</InputEl>
						<ButtomDivEl>
